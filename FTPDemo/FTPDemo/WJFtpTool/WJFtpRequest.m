...
- (void)downloadRequest
{
  ...
  //    [self performSelector:@selector(scheduleInCurrentThread:) onThread:[[self class] networkThreadSpare] withObject:self.dataInputStream waitUntilDone:YES];
...
//    [self stopReceiveSucess:YES info:self.remotePath];
    [self stopUploadReceiveSucess:YES info:self.remotePath];
  } else {
    NSInteger   bytesWritten;//实际写入数据
...
/// 针对上传文件处理
- (void)stopUploadReceiveSucess:(BOOL)sucess info:(NSString *)info {
  WJLog(@"关闭连接");
  _listSucessBlock = nil;
  _listFailBlock = nil;
                                ...
                                fail:(failAction)fail
{
  ...
  self.isDownLoad = YES;
  _totalSize = self.downLaodFileSize;
...
+ (NSThread *)networkThreadOther {
  static NSThread * networkThreadOther = nil;
  static dispatch_once_t multiThreadSingletonOther;
  dispatch_once(&multiThreadSingletonOther, ^{
    networkThreadOther = [[NSThread alloc] initWithTarget:self selector:@selector(networkThreadMain) object:nil];
    [networkThreadOther start];
  });
  return networkThreadOther;
}

+ (NSThread *)networkThreadOtherOne {
  static NSThread * networkThreadOtherOne = nil;
  static dispatch_once_t multiThreadSingletonOtherOne;
  dispatch_once(&multiThreadSingletonOtherOne, ^{
    networkThreadOtherOne = [[NSThread alloc] initWithTarget:self selector:@selector(networkThreadMain) object:nil];
    [networkThreadOtherOne start];
  });
  return networkThreadOtherOne;
}

+ (NSThread *)networkThreadOtherTwo {
  static NSThread * networkThreadOtherTwo = nil;
  static dispatch_once_t multiThreadSingletonOtherTwo;
  dispatch_once(&multiThreadSingletonOtherTwo, ^{
    networkThreadOtherTwo = [[NSThread alloc] initWithTarget:self selector:@selector(networkThreadMain) object:nil];
    [networkThreadOtherTwo start];
  });
  return networkThreadOtherTwo;
}
...
- (NSMutableArray *)spareThreadArray {
  if (_spareThreadArray == nil) {
    ...
    [_spareThreadArray addObject:[[self class] networkThreadOther]];
    [_spareThreadArray addObject:[[self class] networkThreadOtherOne]];
    [_spareThreadArray addObject:[[self class] networkThreadOtherTwo]];
  }
